...
class TextDataset(Dataset):
    ...
    def __init__(self, data_file: str, train: bool = True, sp_model_prefix: str = None,
        ...
        """
        train_texts, val_texts = sklearn.model_selection.train_test_split(texts, test_size=self.VAL_RATIO, random_state=self.TRAIN_VAL_RANDOM_SEED)
    ...
    def __getitem__(self, item: int) -> Tuple[torch.Tensor, int]:
        ...
        """
        sequence = [self.bos_id] + self.indices[item] + [self.eos_id]
        length = len(sequence)
        ...
        if length > self.max_length:
            ...
            sequence += [self.pad_id] * (self.max_length - length)
